...
class FoursquareAuthViewController: UIViewController {
    ...
    fileprivate let kFoursquareAuthUrlFormat = "https://foursquare.com/oauth2/authenticate?client_id=%@&response_type=token&redirect_uri=%@"
    ...
    override func viewDidLoad() {
        ...
        // Encode URL
        let authURLString = NSString(format: kFoursquareAuthUrlFormat as NSString, self.clientId, self.callback)
...
extension FoursquareAuthViewController: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        // Auth failed
        UIApplication.shared.isNetworkActivityIndicatorVisible = false
        delegate?.foursquareAuthViewControllerDidFail?(error as NSError)
    }
}
